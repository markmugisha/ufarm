...
router.get('/produce/update/:id', connectEnsureLogin.ensureLoggedIn(), async (req, res) =>{
	req.session.user = req.user
	try {
		const updateProduct = await Pdtupload.findOne({_id:req.params.id});
		res.render('updateProduce',{product:updateProduct, currentUser:req.session.user});
